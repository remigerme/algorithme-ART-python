...
def main():
    ...
    def ART_custom(f0, A, R, N_ITER, aleatoire, cst, chemin):
        ...
        schema_acces = "alea" if aleatoire else "succ"
    ...
    chemin = "simulations/varier_vitesse_cv_schema_acces/lapin/"
    ...
    f0 = zeros(L * H)
    f = ART_custom(f0, A, R, int(1e6), False, cst, chemin)
    print("Image \"successive\" terminée")


    # Schéma d'accès aléatoire
    f0 = zeros(L * H)
    f = ART_custom(f0, A, R, int(1e6), True, cst, chemin)
...
def extraire_stats(fichier_ref, chemin, taille):
    ...
    re_filtrer = re.compile("(?P<taille>[0-9]+)_(?P<n_t>[0-9]+)_(?P<n_r>[0-9]+)_(?P<alea>succ|alea)_(?P<n_iter>[0-9]+)\.png")
...
for image in ["slp", "lapin"]:
    ...
    chemin = "simulations/varier_vitesse_cv_schema_acces/{}/".format(image)
